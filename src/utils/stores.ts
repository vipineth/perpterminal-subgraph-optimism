...
import { _getDayId, _getHourId, _getWeekId } from ".";
import {
  Maker,
  Market,
  MarketDaily,
...
export function getOrCreateProtocol(): Protocol {
  ...
  if (!protocol) {
    ...
    protocol.totalTrades = BI_ZERO;
    protocol.uniqueTraders = BI_ZERO;
    protocol.save();
  }
  return protocol;
}
export function getOrCreateProtocolDaily(timestamp: BigInt): ProtocolDaily {
  const protocolId = _getDayId(timestamp);
  let protocol = ProtocolDaily.load(protocolId);
  if (!protocol) {
    protocol = new ProtocolDaily(protocolId);
    protocol.tradingFee = BD_ZERO;
    protocol.tradingVolume = BD_ZERO;
    protocol.totalValueLocked = BD_ZERO;
    protocol.badDebt = BD_ZERO;
...
export function getOrCreateProtocolHourly(timestamp: BigInt): ProtocolHourly {
  const protocolId = _getHourId(timestamp);
  let protocol = ProtocolHourly.load(protocolId);
  if (!protocol) {
    protocol = new ProtocolHourly(protocolId);
    protocol.tradingFee = BD_ZERO;
    protocol.tradingVolume = BD_ZERO;
    protocol.totalValueLocked = BD_ZERO;
    protocol.badDebt = BD_ZERO;
...
export function getOrCreateProtocolWeekly(timestamp: BigInt): ProtocolWeekly {
  const protocolId = _getWeekId(timestamp);
  let protocol = ProtocolWeekly.load(protocolId);
  if (!protocol) {
    protocol = new ProtocolWeekly(protocolId);
    protocol.tradingFee = BD_ZERO;
    protocol.tradingVolume = BD_ZERO;
    protocol.totalValueLocked = BD_ZERO;
    protocol.badDebt = BD_ZERO;
...
export function getOrCreateDailyMarket(
  baseToken: Address,
  timestamp: BigInt
): MarketDaily {
  const marketId = formatMarketId(baseToken)
    .concat(`-`)
    .concat(_getDayId(timestamp));
  let market = MarketDaily.load(marketId);
  if (!market) {
    market = new MarketDaily(marketId);
    market.baseToken = Address.fromString(ADDRESS_ZERO);
    market.quoteToken = Address.fromString(ADDRESS_ZERO);
    market.pool = Address.fromString(ADDRESS_ZERO);
    market.feeRatio = BI_ZERO;
...
export function getOrCreateTrader(traderAddr: Address): Trader {
  ...
  if (!trader) {
    ...
    trader.referrerCode = "";
    // if (addTrader) {
    //   let protocol = Protocol.load("perpetual-protocol");
    //   if (protocol) {
    //     protocol.uniqueTraders = protocol.uniqueTraders.plus(BI_ONE);
    //     protocol.save();
    //   }
    // }
    trader.save();
...
export function saveToPositionHistory(
  position: Position,
  event: ethereum.Event
): void {
  const positionHistoryId =
    position.id +
    "-" +
...
export function getReferralCodeTraderDayData(
  dayDataId: string,
  trader: string
): ReferralCodeTraderDayData {
