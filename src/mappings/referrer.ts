  ...
  if (existingReferralCode) {
    ...
    if (event.params.newReferralCode != "") {
      ...
      if (event.params.action == 2) {
        ...
        if (event.params.oldReferralCode == event.params.newReferralCode)
          return;
      }
...
export function handleUncappedPartnerUpserted(
  event: OnUncappedPartnerAssigned
): void {
  let trader = getOrCreateTrader(event.params.addr);
...
export function handleUncappedPartnerRemoved(
  event: OnUncappedPartnerRemoved
): void {
  let trader = getOrCreateTrader(event.params.addr);
  let referrerCode = trader.referrerCode;
