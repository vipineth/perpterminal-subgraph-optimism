...
export function handleDeposited(event: DepositedEvent): void {
  ...
  const increasedTraderBadDebt = getBadDebt(trader.collateral).minus(
    oldTraderBadDebt
  );
  ...
  if (deposited.collateralToken == USDCAddress) {
    protocol.totalValueLocked = protocol.totalValueLocked.plus(
      deposited.amount
    );
  } else {
  ...
  protocolHourly.badDebt = protocolHourly.badDebt.plus(increasedTraderBadDebt);
  if (deposited.collateralToken == USDCAddress) {
    protocolHourly.totalValueLocked = protocolHourly.totalValueLocked.plus(
      deposited.amount
    );
  } else {
    // TODO: non-USD collateral
  ...
  protocolDaily.badDebt = protocolDaily.badDebt.plus(increasedTraderBadDebt);
  if (deposited.collateralToken == USDCAddress) {
    protocolDaily.totalValueLocked = protocolDaily.totalValueLocked.plus(
      deposited.amount
    );
  } else {
    // TODO: non-USD collateral
  ...
  protocolWeekly.badDebt = protocolWeekly.badDebt.plus(increasedTraderBadDebt);
  if (deposited.collateralToken == USDCAddress) {
    protocolWeekly.totalValueLocked = protocolWeekly.totalValueLocked.plus(
      deposited.amount
    );
  } else {
    // TODO: non-USD collateral
...
export function handleWithdrawn(event: WithdrawnEvent): void {
  ...
  const protocol = getOrCreateProtocol();
  protocol.badDebt = protocol.badDebt.plus(increasedTraderBadDebt);
  if (withdrawn.collateralToken === USDCAddress) {
    protocol.totalValueLocked = protocol.totalValueLocked.minus(
      withdrawn.amount
    );
  } else {
    // TODO: non-USD collateral
