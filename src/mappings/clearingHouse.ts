...
import {
  ...
  getOrCreateDailyMarket,
  getOrCreateMaker,
  getOrCreateMarket,
  getOrCreateOpenOrder,
  getOrCreatePosition,
  getOrCreateProtocol,
  getOrCreateProtocolDaily,
  getOrCreateProtocolHourly,
  getOrCreateProtocolWeekly,
  getOrCreateTrader,
  getReferralCode,
...
export function handlePositionChanged(event: PositionChangedEvent): void {
  ...
  positionChanged.exchangedPositionSize = fromWei(
    event.params.exchangedPositionSize
  );
  positionChanged.exchangedPositionNotional = fromWei(
  ...
  positionChanged.marketPriceAfter = fromSqrtPriceX96(
    event.params.sqrtPriceAfterX96
  );
  const functionSignature = event.transaction.input.slice(0, 4);
  positionChanged.fromFunctionSignature = Bytes.fromUint8Array(
    functionSignature
  );
  ...
  if (positionChanged.exchangedPositionSize.equals(BD_ZERO)) {
    ...
    positionChanged.swappedPrice = abs(
      positionChanged.exchangedPositionNotional.div(
        positionChanged.exchangedPositionSize
      )
    );
  }
  ...
  protocol.tradingVolume = protocol.tradingVolume.plus(
  ...
  protocol.totalTrades = protocol.totalTrades.plus(BI_ONE);

  // upsert protocol hourly
  const protocolHourly = getOrCreateProtocolHourly(event.block.timestamp);
  protocolHourly.tradingVolume = protocolHourly.tradingVolume.plus(
    abs(positionChanged.exchangedPositionNotional)
  );
  protocolHourly.tradingFee = protocolHourly.tradingFee.plus(
    positionChanged.fee
  );
  protocolHourly.timestamp = event.block.timestamp;
  protocolHourly.totalTrades = protocolHourly.totalTrades.plus(BI_ONE);
  protocolHourly.cumulativeVolume = protocol.tradingVolume;

  // upsert protocol daily
  const protocolDaily = getOrCreateProtocolDaily(event.block.timestamp);
  protocolDaily.tradingVolume = protocolDaily.tradingVolume.plus(
    abs(positionChanged.exchangedPositionNotional)
  );
  protocolDaily.tradingFee = protocolDaily.tradingFee.plus(positionChanged.fee);
  protocolDaily.timestamp = event.block.timestamp;
  protocolDaily.totalTrades = protocolDaily.totalTrades.plus(BI_ONE);
  protocolDaily.cumulativeVolume = protocol.tradingVolume;

  // upsert protocol weekly
  const protocolWeekly = getOrCreateProtocolWeekly(event.block.timestamp);
  protocolWeekly.tradingVolume = protocolWeekly.tradingVolume.plus(
    abs(positionChanged.exchangedPositionNotional)
  );
  protocolWeekly.tradingFee = protocolWeekly.tradingFee.plus(
    positionChanged.fee
  );
  protocolWeekly.timestamp = event.block.timestamp;
  protocolWeekly.totalTrades = protocolWeekly.totalTrades.plus(BI_ONE);
  protocolWeekly.cumulativeVolume = protocol.tradingVolume;

  // upsert Market
  const market = getOrCreateMarket(event.params.baseToken);
  market.blockNumber = event.block.number;
  market.timestamp = event.block.timestamp;
  market.tradingVolume = market.tradingVolume.plus(
    abs(positionChanged.exchangedPositionNotional)
  );
  market.tradingFee = market.tradingFee.plus(positionChanged.fee);

  //   upsert Market
  const marketDaily = getOrCreateDailyMarket(
    event.params.baseToken,
    event.block.timestamp
  );
  marketDaily.blockNumber = event.block.number;
  marketDaily.timestamp = event.block.timestamp;
  marketDaily.tradingVolume = marketDaily.tradingVolume.plus(
    abs(positionChanged.exchangedPositionNotional)
  );
  marketDaily.tradingFee = marketDaily.tradingFee.plus(positionChanged.fee);
  marketDaily.baseToken = event.params.baseToken;
  ...
  trader.tradingVolume = trader.tradingVolume.plus(
    abs(positionChanged.exchangedPositionNotional)
  );
  ...
  position.positionSize = position.positionSize.plus(
    positionChanged.exchangedPositionSize
  );
  ...
  position.tradingVolume = position.tradingVolume.plus(
    abs(positionChanged.exchangedPositionNotional)
  );
  ...
  traderDayData.tradingVolume = traderDayData.tradingVolume.plus(
    abs(positionChanged.exchangedPositionNotional)
  );
  traderDayData.fee = traderDayData.fee.plus(event.params.fee);
  traderDayData.realizedPnl = traderDayData.realizedPnl.plus(
    event.params.realizedPnl
  );
...
export function handlePositionLiquidated(event: PositionLiquidatedEvent): void {
  ...
  positionLiquidated.positionNotionalAbs = fromWei(
  ...
  position.blockNumber = event.block.number;
  position.timestamp = event.block.timestamp;
  position.liquidationFee = position.liquidationFee.plus(
    positionLiquidated.liquidationFee
  );
  ...
  trader.blockNumber = event.block.number;
  trader.timestamp = event.block.timestamp;
  trader.liquidationFee = trader.liquidationFee.plus(
    positionLiquidated.liquidationFee
  );
...
export function handleLiquidityChanged(event: LiquidityChangedEvent): void {
  ...
  if (openOrder.liquidity === BI_ZERO) {
    ...
    openOrder.collectedFeeInThisLifecycle = openOrder.collectedFeeInThisLifecycle.plus(
      liquidityChanged.quoteFee
    );
  }
  openOrder.collectedFee = openOrder.collectedFee.plus(
    liquidityChanged.quoteFee
  );
  ...
  marketDaily.baseAmount = marketDaily.baseAmount.plus(liquidityChanged.base);
  marketDaily.quoteAmount = marketDaily.quoteAmount.plus(
    liquidityChanged.quote
  );
...
export function handleFundingPaymentSettled(
  ...
  position.blockNumber = event.block.number;
  position.timestamp = event.block.timestamp;
  position.fundingPayment = position.fundingPayment.plus(
    fundingPaymentSettled.fundingPayment
  );
  ...
  trader.blockNumber = event.block.number;
  trader.timestamp = event.block.timestamp;
  trader.fundingPayment = trader.fundingPayment.plus(
    fundingPaymentSettled.fundingPayment
  );
...
export function handleReferralPositionChanged(
  ...
  let positionChangedEvent = PositionChanged.load(
    event.transaction.hash.toHexString() +
      "-" +
      positionChangedLogIndex.toString()
  );
  ...
  if (positionChangedEvent !== null) {
    ...
    if (referralCode !== null) {
      ...
      let referralCodeTraderDayData = getReferralCodeTraderDayData(
      ...
      referralCodeDayData.fees = referralCodeDayData.fees.plus(
        positionChangedEvent.fee
      );
      ...
      referralCodeTraderDayData.fees = referralCodeTraderDayData.fees.plus(
        positionChangedEvent.fee
      );
